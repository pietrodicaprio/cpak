...
- Status: Phases 0 through 5 complete on upstream baseline `38fa798` (`v2.9.7`);
  Phase 6 planned
- Baseline: certified code `a17078f`, semantic merge `d7f9034`, upstream
  `38fa798`
...
The dedicated `integration/application-trust-e86fa23` branch carries the 18
upstream commits after `be29d55`, including the `v2.9.7` package-authority
hardening, Manifest v3 Phase 5 fixtures, and semantic resolutions for both the
original 14 trust-sensitive conflicts and the three content conflicts added by
the hardening refresh. Local Linux test, race, vet, and lifecycle evidence is
recorded in `application-trust-upstream-integration.md`. Both Phase 5 jobs and
every portability job pass at certified code commit `a17078f` in
[Portability run 33168708481](https://github.com/pietrodicaprio/cpak/actions/runs/33168708481),
so the upstream recertification gate is complete.
